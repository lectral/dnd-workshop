...
Use this schema to build adventures for Chronicles. The root object accepts four top-level keys: `meta`, `counters`, `inventory`, and `scenes`.
...
- `bonus_timing` is only used by `bonus` items and must be `before`, `after`, or `both`. Missing timing defaults to `both`.
...
- `short_text`: compact label on the exit button
